...
TEST(Vector3_tests, Constructors) {
	...
	for (int i = 0; i < 100; ++i) {
		...
		const Math::Vector3<int> vector(x, y, z);
	...
	for (int i = 0; i < 100; ++i) {
		...
		const Math::Vector3<float> vector(x, y, z);
		EXPECT_EQ(vector.mX, x);
		EXPECT_EQ(vector.mY, y);
		EXPECT_EQ(vector.mZ, z);
	}
	...
	for (int i = 0; i < 100; ++i) {
		...
		const Math::Vector3<double> vector(x, y, z);
		EXPECT_EQ(vector.mX, x);
		EXPECT_EQ(vector.mY, y);
		EXPECT_EQ(vector.mZ, z);
	}
...
TEST(Vector3_tests, Assignment) {
	for (int i = 0; i < 100; i++) {
		...
		const Math::Vector3<float> originalVec(x, y, z);
	...
	for (int i = 0; i < 100; i++) {
		...
		const Math::Vector3<double> originalVec(x, y, z);
		auto copyVector = originalVec;
		EXPECT_EQ(copyVector.mX, x);
		EXPECT_EQ(copyVector.mY, y);
		EXPECT_EQ(copyVector.mZ, z);
	...
	for (int i = 0; i < 100; i++) {
		...
		const Math::Vector3<int> originalVec(x, y, z);
		auto copyVector = originalVec;
		EXPECT_EQ(copyVector.mX, x);
		EXPECT_EQ(copyVector.mY, y);
		EXPECT_EQ(copyVector.mZ, z);
...
TEST(Vector3_tests, Addition) {
	for (int i = 0; i < 100; i++) {
		...
		const Math::Vector3<float> vector1(x1, y1, z1);
		const Math::Vector3<float> Vector3(x2, y2, z2);
		...
		Math::Vector3<float> vector4(x3, y3, z3);
	...
	for (int i = 0; i < 100; i++) {
		...
		const Math::Vector3<double> vector1(x1, y1, z1);
		const Math::Vector3<double> Vector3(x2, y2, z2);
		const auto resultVector = vector1 + Vector3;
		...
		Math::Vector3<double> vector4(x3, y3, z3);
		vector4 += resultVector;
	...
	for (int i = 0; i < 100; i++) {
		...
		const Math::Vector3<int> vector1(x1, y1, z1);
		const Math::Vector3<int> Vector3(x2, y2, z2);
		const auto resultVector = vector1 + Vector3;
		...
		Math::Vector3<int> vector4(x3, y3, z3);
		vector4 += resultVector;
...
TEST(Vector3_tests, LengthSquared) {
	for (int i = 0; i < 100; i++) {
		...
		const Math::Vector3<float> vector(x1, y1, z1);
	...
	for (int i = 0; i < 100; i++) {
		...
		const Math::Vector3<double> vector(x1, y1, z1);
		const auto lengthSqrd = vector.LengthSqrd();
		const auto expectedValue = x1 * x1 + y1 * y1 + z1 * z1;
	...
	for (int i = 0; i < 100; i++) {
		...
		const Math::Vector3<int> vector(x1, y1, z1);
...
TEST(Vector3_tests, GetNormalized) {
	for (int i = 0; i < 100; i++) {
		...
		const auto length = vector.Length();
		const Math::Vector3<float> expectedValue(
			vector.mX / length,
			vector.mY / length,
			vector.mZ / length);
	...
	for (int i = 0; i < 100; i++) {
		...
		const auto normalizedVector = vector.GetNormalized();
		const auto length = vector.Length();
		const Math::Vector3<double> expectedValue(
			vector.mX / length,
			vector.mY / length,
			vector.mZ / length);
	...
	for (int i = 0; i < 100; i++) {
		...
		const auto normalizedVector = vector.GetNormalized();
		const auto length = vector.Length();
		const Math::Vector3<int> expectedValue(
			vector.mX / length,
			vector.mY / length,
			vector.mZ / length);
...
TEST(Vector3_tests, Length_Zero) {
	Math::Vector3<float> vectorF;
	EXPECT_EQ(vectorF.Length(), 0.0f);

	Math::Vector3<double> vectorD;
	EXPECT_EQ(vectorD.Length(), 0.0);

	Math::Vector3<int> vectorI;
...
TEST(Vector3_tests, Normalize_Zero) {
	Math::Vector3<float> vectorF;
	vectorF.Normalize();
	Math::Vector3<float> normalizedVectorF = vectorF.GetNormalized();
	EXPECT_EQ(vectorF.Length(), 0.0f);
	EXPECT_EQ(normalizedVectorF.Length(), 0.0f);

	Math::Vector3<double> vectorD;
	vectorD.Normalize();
	Math::Vector3<double> normalizedVectorD = vectorD.GetNormalized();
	EXPECT_EQ(vectorD.Length(), 0.0);
	EXPECT_EQ(normalizedVectorD.Length(), 0.0);

	Math::Vector3<int> vectorI;
	vectorI.Normalize();
	Math::Vector3<int> normalizedVectorI = vectorI.GetNormalized();
	EXPECT_EQ(vectorI.Length(), 0);
...
TEST(Vector3_tests, Cross) {
	for (int i = 0; i < 100; i++) {
		...
		const Math::Vector3<float> controlCross = {
			y1 * z2 - z1 * y2, 
			z1 * x2 - x1 * z2,
			x1 * y2 - y1 * x2};
	...
	for (int i = 0; i < 100; i++) {
		...
		const Math::Vector3<double> controlCross = {
			y1 * z2 - z1 * y2,
			z1 * x2 - x1 * z2,
			x1 * y2 - y1 * x2 };
	...
	for (int i = 0; i < 100; i++) {
		...
		const Math::Vector3<int> controlCross = {
			y1 * z2 - z1 * y2,
			z1 * x2 - x1 * z2,
			x1 * y2 - y1 * x2 };
